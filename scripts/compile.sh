...
Initialises vendored submodules and builds the simple module plugin. Defaults to Release.
...
echo "Building simple_module_plugin..."
cmake --build "${BUILD_DIR}" --target simple_module_plugin --config "${BUILD_TYPE}"
...
BIN_PATH="${BUILD_DIR}/modules/simple_module_plugin${LIB_SUFFIX}"
if [[ -f "${BIN_PATH}" ]]; then
